module.exports = class extends think.Model {
  ...
  async get_channel_cache() {
    const channel = await think.cache(
      'get_channel_cache',
      () => {
  ...
  async get_channel(id) {
    ...
    for (const v of list) {
      ...
      if (v.url.indexOf('http://') == -1 || v.url.indexOf('https://') == -1) {
        if (!think.isEmpty(v.url) && think.isString(v.url)) {
          ...
          if (!think.isNumberString(name) && !think.isEmpty(name)) {
            name = await this.model('category')
              .where({ name: name })
              .getField('id', true);
          }
        }
      ...
      if (name != 0 && !think.isEmpty(name)) {
        const subcate = await this.model('cmswing/category').get_sub_category(
          name
        );
        subcate.push(name);
        v.on = subcate;
